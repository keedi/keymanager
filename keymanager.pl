...
=head2 /host/:name/key/delete

해당 호스트의 키를 삭제하도록 서버에 요청합니다.

=cut

get "/host/:name/key/delete" => sub {
    my $c = shift;
    $c->render( template => "host-name-key-delete" );
};

=head2 /user/:name/history
...
get "/user/:name/history" => sub {
    my $c = shift;
    $c->render( template => "user-name-history" );
};
